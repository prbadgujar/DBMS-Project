...
const ThingsTodo= () => {
  ...
  useEffect(() => {
   ...
   fetch(`http://localhost:8080/api/locations/activity/parkId/${nationalPark}`)
...
const handleSubmit = () => {
  ...
  // setNpInfo((prev) => (prev === info ? "" : info)); // Toggle selection
    // navigate(`/activity/parkId/${nationalPark}`);
    // Perform navigation or other actions here
  }
  ...
  return (
   <div className="container">
      ...
      {npInfo.map((np) => (
       ...
       <label>
       <ul>
<li>Name: {np.thingsName}</li>
<li>Description: {np.thingsDescription}</li>
<li>Types: {np.thingsType}</li>
<li>
  <a href={np.thingsLocation} target="_blank">location
    </a></li>
</ul>
       </label>
     </div>     
